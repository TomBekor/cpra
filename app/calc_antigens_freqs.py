...
def parse_input():
    path = 'input'
    loci_list = ['A', 'B', 'C', 'DQB1', 'DRB1', 'DPB1']
...
def find_haps_with_antigens(graph, antigens_list):
    ...
    return sum(dict_haps_with_antigens.values()),  sum(dict_haps_with_antigens_class1.values()),\
           sum(dict_haps_with_antigens_class2.values()), len(dict_haps_with_antigens_class1), len(dict_haps_with_antigens_class2)
...
def call_calc_antigens(pop = 'General_IL', string_input = None, Assume_HWE=False):
    ...
    file_res = open('output/probs_without_antigens_haps.csv', 'w')
    if string_input:
        dict_donors = {}
    ...
    for sample_id, antigens_list in dict_donors.items():
            ...
            freq_positive_all, freq_positive_class1, freq_positive_class2 = \
                int(round(freq_positive_all,2)*100), int(round(freq_positive_class1,2)*100),int(round(freq_positive_class2,2)*100)
            file_res.write("{id},{freq_all}%,{freq_class1}%,{freq_class2}%\n".format
                           ( id=sample_id,freq_all=freq_positive_all,freq_class1=freq_positive_class1,freq_class2=freq_positive_class2))
